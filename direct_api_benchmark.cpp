...
public:
    DirectAPIBenchmark(size_t insert_records = 10000000) 
        ...
        std::cout << "Direct API Benchmark - " << INSERT_RECORDS << " records" << std::endl;
    ...
    void initializeTree() {
        ...
        // Initialize NVM pool (1GB for 10M records)
        const char* nvm_filename = "/tmp/direct_api_benchmark.nvm";
        long long nvm_size = 1024 * 1024 * 1024; // 1GB
        
        // Clean up any existing NVM file
        std::system("rm -f /tmp/direct_api_benchmark.nvm");
    ...
    void benchmarkPureInsert() {
        ...
        std::cout << "Inserting " << (INSERT_RECORDS-1) << " records..." << std::endl;
        
        auto start = std::chrono::high_resolution_clock::now();
        
        // Direct API calls to insert (skip first key since it's already bulkloaded)
        for (size_t i = 1; i < INSERT_RECORDS; i++) {
